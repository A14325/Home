...
# print(x.shape, y.shape)  #***array([0, 1, 2]), array([50, 50, 50]))
...
x_train, x_test, y_train, y_test = train_test_split(x, y,                                                  #데이터 분할 
    train_size = 0.75,
    random_state = 333,
    shuffle=True)         #몇대 몇으로 분리되는지도 꼭 확인할것
    
    
    
print("x값:", x_train.shape, x_test.shape)    # (112, 4) (38, 4)
print("y값:", y_train.shape, y_test.shape)      #(112,3) (38,3) -y값을 onehotencoding 형식으로 바꿔줘야함
exit()
print(np.unique(y_train, return_counts=True)) #(array([0, 1, 2]), array([37, 37, 38]))
#exit()  
...
model.add(Dense(3, activation='softmax'))  # ***다중분류이기에 무조건 softmax 
#마지막 레이어 노드=맞춰야할 클래스의 개수
